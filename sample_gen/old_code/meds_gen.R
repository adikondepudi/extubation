#Alexandra Szewc
#To create sample dataframe for reintubations including heart rate and medications

#Alex: 01/21/21 - Created this file
  ...
  for (j in 1:nrow(temp_peep)) {
    ...
    if (peep_now - peep_last > gap_length || j == nrow(temp_peep)) { # check if end of record or reintubation
      if (j == nrow(temp_peep) & peep_val != peep_now) { # handle end of record
        ...
        exists_med <- nrow(temp_meds[which(temp_meds$drugstartoffset > peep_last & temp_meds$drugstartoffset < peep_now),])
        meets_criteria <- (exists_PP == 0 && exists_Etco2 == 0 && exists_TVIBW == 0 && exists_hr != 0 && exists_med != 0) 
...
write.csv(sample_identification_df,paste0(save_dir, "/", gap_length, "_hrMED.csv"), row.names = F)
