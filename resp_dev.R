...
#Alex: 02/03/21 - Added screening for all drugs; removed old meds screen
...
prop <- read.csv(paste0(save_dir, "/allmeds_records.csv"))
...
hr <- hr[which(!is.na(hr$offset) &
              hr$patientunitstayid %in% unique_pids),] %>% 
  group_by(patientunitstayid) %>% 
  arrange(patientunitstayid, offset) %>% droplevels()

# meds <- meds[which((tolower(meds$drugname) %like% "propofol" |
...
#                       & (!is.na(meds$drugstartoffset))
#                       & meds$patientunitstayid %in% unique_pids),] %>% 
#   dplyr::select(patientunitstayid, drugname, drugstartoffset) %>% 
#   group_by(patientunitstayid) %>% 
#   arrange(patientunitstayid, drugstartoffset) %>% droplevels()

prop <- prop[which(prop$patientunitstayid %in% unique_pids),] %>%
  group_by(patientunitstayid) %>% 
  arrange(patientunitstayid, offset) %>% droplevels()

resp <- resp[which(!is.na(resp$offset) & !is.na(resp$value) &
                 resp$patientunitstayid %in% unique_pids),] %>% 
  group_by(patientunitstayid) %>% 
  arrange(patientunitstayid, offset) %>% droplevels()
...
sample_identification_df <- foreach(i = 1:length(unique_pids), .combine=rbind) %dopar% {
  ...
  # temp_meds <- meds[which(meds$patientunitstayid == temp_id), ]
  temp_prop <- prop[which(prop$patientunitstayid == temp_id),]
  temp_resp <- resp[which(resp$patientunitstayid == temp_id), ]
  ...
  for (j in 1:nrow(temp_peep)) {
    ...
    if (peep_now - peep_last > gap_length || j == nrow(temp_peep)) { # check if end of record or reintubation
      if (j == nrow(temp_peep) & peep_val != peep_now) { # handle end of record
        ...
        exists_med <- nrow(temp_prop[which(temp_prop$offset > peep_last & temp_prop$offset < peep_now),])
          #+ nrow(temp_meds[which(temp_meds$drugstartoffset > peep_last & temp_meds$drugstartoffset < peep_now),])
                       
        resp_calc <- temp_resp[which(temp_resp$offset > peep_last & temp_resp$offset < peep_now), ]
        std_resp <- sd(resp_calc$value, na.rm = TRUE)
        if (is.na(std_resp)) {
          std_resp <- 0
        }
        meets_criteria <- (exists_PP == 0 && exists_Etco2 == 0 && exists_TVIBW == 0 && exists_hr != 0 && exists_med != 0 && std_resp > 0) 
...
names(sample_identification_df) <- c("patientunitstayid", "class", "MV_number", "is_last",
                                     ...
                                     "exists_TVIBW", "exists_hr", "exists_med", "std_resp",
                                     "MV_ID", "Meets_criteria")
toc()
stopCluster(cl)

write.csv(sample_identification_df,paste0(save_dir, "/", gap_length, "meds_by_num.csv"), row.names = F)
